...
(() => {
  ...
  el("ollama-save").addEventListener("click", async () => {
    ...
    if (!model) { setStatus("ollama-status", "No model selected.", "error"); return; }
  ...
  el("anthropic-save").addEventListener("click", async () => {
    ...
    if (!key || !key.startsWith("sk-ant-")) { setStatus("anthropic-status", "Invalid key (should start with sk-ant-)", "error"); return; }
  ...
  el("anthropic-test").addEventListener("click", async () => {
    const key = el("anthropic-key").value.trim();
    if (!key) { setStatus("anthropic-status", "Enter a key first.", "error"); return; }
    setStatus("anthropic-status", "Testing...", "");
    try {
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": key, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
        body: JSON.stringify({ model: el("anthropic-model").value, max_tokens: 16, messages: [{ role: "user", content: "Reply OK" }] }),
      });
      if (r.ok) setStatus("anthropic-status", "Connected.", "success");
      else setStatus("anthropic-status", `Error ${r.status}`, "error");
    } catch (e) { setStatus("anthropic-status", `Failed: ${e.message}`, "error"); }
  });
  ...
  el("openai-save").addEventListener("click", async () => {
    ...
    if (!key || !key.startsWith("sk-")) { setStatus("openai-status", "Invalid key (should start with sk-)", "error"); return; }
  ...
  el("openai-test").addEventListener("click", async () => {
    const key = el("openai-key").value.trim();
    if (!key) { setStatus("openai-status", "Enter a key first.", "error"); return; }
    setStatus("openai-status", "Testing...", "");
    try {
      const r = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${key}` },
        body: JSON.stringify({ model: el("openai-model").value, max_tokens: 16, messages: [{ role: "user", content: "Reply OK" }] }),
      });
      if (r.ok) setStatus("openai-status", "Connected.", "success");
      else setStatus("openai-status", `Error ${r.status}`, "error");
    } catch (e) { setStatus("openai-status", `Failed: ${e.message}`, "error"); }
  });
  ...
  el("google-save").addEventListener("click", async () => {
    ...
    if (!key) { setStatus("google-status", "Enter a key.", "error"); return; }
  ...
  el("google-test").addEventListener("click", async () => {
    const key = el("google-key").value.trim();
    if (!key) { setStatus("google-status", "Enter a key first.", "error"); return; }
    ...
    } catch (e) { setStatus("google-status", `Failed: ${e.message}`, "error"); }
  });
  ...
  el("safety-save").addEventListener("click", async () => {
    ...
    if (isNaN(maxTurns) || maxTurns < 1 || maxTurns > 100) { setStatus("safety-status", "Must be 1-100.", "error"); return; }
